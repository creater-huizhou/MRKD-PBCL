    ...
    def _train_epoch(self, epoch, prototypes, count_features):
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            if batch_idx == 0:
                ...
                self.logger.info(f"lr[0]: {self.optimizer.param_groups[0]['lr']:.6f} / lr[1]: {self.optimizer.param_groups[1]['lr']:.6f}")
...
class Trainer_Segformer_base(BaseTrainer):
    ...
    def _update_running_stats(self, labels, features, prototypes, count_features):
        ...
        for cl in cl_present:
            ...
            # cumulative moving average for each feature vector
            # S_{n+f} = ( sum(x_{n+1} + ... + x_{n+f}) + n * S_n) / (n + f)
            features_running_mean_tot_cl = (features_cl_sum + count_features.detach()[cl] * prototypes.detach()[cl]) / (count_features.detach()[cl] + features_cl.shape[0])
    ...
    def _visualize_with_tSNE(self, tSNE_path_to_save, features, labels_down):
        # visualization with t-SNE from sklearn (very slow)
        # can improve 1000x with t-SNE for CUDA or x100 with t-SNE with multiprocessing
        ...
        colors = ['#000000', '#800000', '#008000', '#808000', '#000080', '#800080', '#008080', '#808080', '#400000',
        ...
        sns.scatterplot(x=tsne_result[:, 0],
                        y=tsne_result[:, 1],
                        hue=[classes[x] for x in labels_down.view(-1).cpu().numpy()],
                        legend=False,
                        # palette=sns.color_palette("bright", torch.unique(labels_down).size(0))
                        color=colors[torch.unique(labels_down).size(0)]
                        )
        plt.savefig(tSNE_path_to_save)
        plt.close()
    ...
    def _test(self, epoch=None):
        ...
        total_feature = torch.zeros([len(self.test_loader) * self.total_classes, 256]).to(self.device)
        class_feature = torch.zeros([len(self.test_loader) * self.total_classes]).to(self.device)
        cnt = torch.tensor(0).long().to(self.device)
        with torch.no_grad():
            for batch_idx, data in enumerate(self.test_loader):
                data['image'], data['label'] = data['image'].to(self.device), data['label'].to(self.device)
                target = data['label'].cpu().numpy()

                logit, features, _, _ = self.model(data['image'])
                logit = torch.sigmoid(logit)
                pred = logit[:, 1:].argmax(dim=1) + 1  # pred: [N. H, W]
                ...
                for bs in range(B):
                    # The category in the current mask
                    cl_present = torch.unique(input=labels_down[bs])
                    if cl_present[0] == 0:
                        cl_present = cl_present[1:]
                    if len(cl_present) == 0:
                        continue
                    ...
                    _pl = labels_down.view(-1)  # h*w -> hw
                    feat = einops.rearrange(features[bs], 'c h w -> ( h w ) c ')

                    for cl in cl_present:
                        pl = (_pl == cl)
                        total_feature[cnt] = feat[pl].mean(dim=0)
                        class_feature[cnt] = cl
                        cnt += 1

            print("the number of feature: ", cnt)
            print("waiting for generating the tsne map...")

            # visualize
            save_dir = self.config['trainer']['save_cluster_map_dir']

            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            save_path = os.path.join(save_dir , 'MRKD_PBCL_step_5_TSNE.jpg')
            self._visualize_with_tSNE(save_path, total_feature[:cnt], class_feature[:cnt])
            print("the generation of tsne map is done.")
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=self.config['use_amp']):
                ...
                if self.model_old is not None:
                    with torch.no_grad():
                        logit_old, features_old, layer_features_old, pos_neg_features_old = self.model_old(data['image'], ret_intermediate=True)

                #layer_features.append(logit[:, 1:self.n_old_classes + 1])
                #layer_features_old.append(logit_old[:, 1:])
                ...
                # loss_kd = torch.tensor(0.).to(self.device)
                #loss_dkd_pos = torch.tensor(0.).to(self.device)
                #loss_dkd_neg = torch.tensor(0.).to(self.device)
