...
class BaseTrainer:
    ...
    def __init__(self, config, logger, gpu):
        ...
        self.prototypes = torch.zeros([class_num + 1, decoder_dim])
        self.prototypes.requires_grad = False
        self.count_features = torch.zeros([class_num + 1, 1])
        self.count_features.requires_grad = False
    ...
    def train(self):
        ...
        self.count_features = self.count_features.to(self.device)
        for epoch in range(self.start_epoch, self.epochs + 1):
            result, val_flag, self.prototypes, self.count_features = self._train_epoch(epoch, self.prototypes, self.count_features)
                ...
                if val_flag and (self.mnt_mode != 'off'):
                    try:
                        ...
                        improved = (self.mnt_mode == 'min' and float(log[self.mnt_metric]) < self.mnt_best) or \
                                   (self.mnt_mode == 'max' and float(log[self.mnt_metric]) > self.mnt_best)
                    except KeyError:
    ...
    def _save_checkpoint(self, epoch):
        ...
        if isinstance(self.model, (nn.DataParallel, nn.parallel.DistributedDataParallel)):
            state = {
                ...
                'count_feature': self.count_features
            }
        else:
            state = {
                'arch': arch,
    ...
    def _resume_checkpoint(self, resume_path, test=False):
        ...
        if test is False:
            ...
            self.prototypes = checkpoint['prototype']
            self.count_features = checkpoint['count_feature']
        
        self.logger.info("Checkpoint loaded. Resume training from epoch {}".format(self.start_epoch))

    def _resume_prototype(self, resume_path):
        resume_path = str(resume_path)
        checkpoint = torch.load(resume_path, map_location='cpu')
        self.prototypes = checkpoint['prototype']
        self.count_features = checkpoint['count_feature']

        self.logger.info("Prototype loaded from {}".format(resume_path))
